...
# R version 4.2.2
# 
# ALTERNATE MODEL SPECIFICATION WITH TIME AS A FACTOR VARIABLE RATHER THAN SPLINE
#
...
#### Set up ----
    ...
    outputdir <- "HUD HEARS Study/wage_analysis/output/hpd_revision_1/alt_time_as_factor/"
...
#### Two stage regression with IPTW: wages as outcome of interest ----
  ...
  # Step 3: Fit the mixed model, using the IPTW as a weight, and get predictions ----
      message('Using IPTW as a weight, vs as a covariate, is more robust to misspecification.')
        ...
        model.data[, time := factor(time, levels = c(0, -4:-1, 1:4))]
  ...
  # Step 4: plot ----
      setDT(model.data)
      wage_model.preds_tidy[, time := as.integer(as.character(time))]
...
#### Two regression with IPTW: % AMI as outcome of interest----
  ...
  # Step 4: plot ----
      setDT(model.data)
      ami_model.preds_tidy[, time := as.integer(as.character(time))]
...
#### Sensitivity analysis dropping those with disabilities: two stage regression with IPTW: wages as outcome of interest ----
  ...
  # Step 3: Fit the mixed model, using the IPTW as a weight, and get predictions ----
      ...
      addWorksheet(wb, 'Table_X_regression_non-disabled') 
      writeDataTable(wb, sheet = 'Table_X_regression_non-disabled', sensitivity_model.tidy, 
                     rowNames = F, colNames = T)   
  ...
  # Step 4: plot ----
      setDT(model.data)
      sensitivity_model.preds_tidy[, time := as.integer(as.character(time))]
      ...
      sensitivity_model.plot <- formatplots(sensitivity_model.plot) + 
        scale_y_continuous(limits = c(4000, 9500), breaks=c(seq(4000, 9000, 1000)), labels=scales::dollar_format()) +
        facet_wrap(~program, nrow = 2, strip.position = "top") +
        theme(panel.spacing = unit(15, "pt"), 
