...
# Model 1: Model for all available quarterly wage data ----
    ...
    for(ii in c("KCHA", "TBV", "PBV", "PH")){
      # get confounder vars ----
          if(ii == "KCHA"){confounders <- confounders <-"exit_year + hh_disability + prog_type_use + housing_time_at_exit"}
          if(ii != "KCHA"){confounders <- confounders <-"exit_year + hh_disability + housing_time_at_exit"}
      
      # Create dt1 for complete quarterly analysis ----
          if(ii != "KCHA"){dt1 <- copy(raw)[qtr %in% -4:4 & prog_type_use == ii]}
          if(ii == "KCHA"){dt1 <- copy(raw)[qtr %in% -4:4 & prog_type_use != "All KCHA"]}
    ...
    # Plot data four quarters before and after exit ----
      ...
      mod1.preds[, program := factor(program, 
                                           levels = c("KCHA", "TBV", "PBV", "PH"), 
                                           labels = c("All KCHA", "TBV", "PBV", "PH"))]
      plot1 <- ggplot() +
