...
# Load data ----
    ...
    raw <- rbind(copy(raw), copy(raw)[, prog_type_use := "All SHA"])[!is.na(prog_type_use)]
    raw[, prog_type_use := factor(prog_type_use, levels = c("All SHA", "TBV", "PBV", "PH"))]
  ...
  # loop over each program type to create series of table2 ----
    for(tempx in sort(unique(raw$prog_type_use))){
      ...
      # Identify confounders ----
        ...
        name.4.save <- gsub("All SHA", "SHA", tempx)
