...
# Load data ----
    ...
    # ESD wage data ----
      message("Updated table is called 'stage_sow1_wage', previous was 'stage_sow1_wages'")
      wage <- setDT(DBI::dbGetQuery(conn = hhsaw20, 
                                    "SELECT id_esd, yr, qtr, wages, hrs 
                                     FROM [esd].[stage_sow1_wage]
    ...
    # ESD address data ----
      ...
      # merge on geocode 
        ...
        esd_geo <- esd_geo[, .(id_esd, start_date, end_date, geo_id10_county, geo_id10_tract, geo_id10_state)]
        rm(geocodes); rm(esd_address)  
      
        
    # HUDHEARS <> ESD crosswalk ----
      xwalk <- setDT(DBI::dbGetQuery(conn = hhsaw20, 
                                    "SELECT DISTINCT id_esd, id_hudhears 
                                     FROM [esd].[stage_sow1_wage]
                                     WHERE wages IS NOT NULL"))
    # PHA exit data ----
...
# Tidy datasets ----
    ...
    # ESD address data (esd_geo) ----  
      esd_geo <- unique(esd_geo[id_esd %in% wage$id_esd]) # only useful if IDs are in the wage data 

    # HUDHEARS <> ESD crosswalk (xwalk) ----  
      # de-duplicate in agnostic manner if there are duplicate hudhears or esd ids
      xwalk[!is.na(id_esd), dup := 1:.N, id_hudhears]
      xwalk <- xwalk[dup == 1]
      xwalk[, dup := NULL]
      
      xwalk[!is.na(id_hudhears), dup := 1:.N, id_esd]
      xwalk <- xwalk[dup == 1]
      xwalk[, dup := NULL]
...
# Merge main datasets ----
    # merge crosswalk onto exits to get id_esd (create 'combo')----
      ...
      # limit to working age individuals 
        ...
        consort05_side_match_id_esd <- nrow(combo[is.na(id_esd)])
    ...
    # merge ESD address info onto ESD wage data accounting for specific time windows ----
        ...
        setkey(esd_geo, id_esd, start_date, end_date)
...
# Create consort diagram ----
    ...
    # create consort diagram ----
      consort.complete <- 
        ...
        add_side_box(txt = paste0("Could not link to wages: ", consort07_side_wages))   |>
